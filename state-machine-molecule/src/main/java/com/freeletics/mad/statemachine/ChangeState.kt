...
internal fun <S> ChangeState<S>.reduce(state: MutableState<S>) {
    synchronized(state) {
        when (val change = this) {
            is MutateState<*, S> -> state.value = change.reduceImpl(state.value)
            is OverrideState -> state.value = change.newState
            is NoStateChange -> {}
        }
    }
}
